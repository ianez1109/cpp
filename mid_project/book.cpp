...
//Initializer without any argument
Book::Book()
...
//function used to make a book available/unavailable
bool Book::borrow_book() {
    if (is_available()) available = false;
    return available;
}

//function to set the book to be available
...
bool Book::check_isbn() {
    for (int i = 0; i < 4; i++) {
        if (i < 3) {
            string token = isbn.substr(i, isbn.find('-'));  //gets first part of the isbn
...
//overload of the "==" operator that checks if two books are the same based on their isbn
bool Book::operator==(const Book& book) const {
    return isbn == book.get_isbn();
}

//overload of the "!=" operator that checks if two books are NOT the same based on their isbn
bool Book::operator!=(const Book& book) const {
    return isbn != book.get_isbn();
}

//overload of the ostream "<<" operator to output the book informations 
